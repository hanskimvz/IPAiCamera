...
settingList = [ "ae_en", "exp_level", "a_flicker", "ae_meter", "awb_en", "awb_mode", "awb_method", "r_gain", "g_gain", "b_gain", "hue", "saturation", "brightness", "contrast", "sharpness", "mctf3dnr", "local_exp", "blc", "hdr", "hdr_level", "dnmode", "shutter","shutter_limit", "sshutter", "mirror", "flip", "thermal_color", "cam_gain","tdn_bw_level","tdn_color_level","tdn_resp","ir_enabled","thermal_color", "thermal_inverse", "thermal_agc", "thermal_mgcc", "thermal_mgcb", "thermal_edge_mode", "thermal_edge_level","thermal_limit", "thermal_shutter_mode", "thermal_shutter_time", "thermal_shutter_temp", "thermal_gamma", "thermal_denoise", "thermal_image", "thermal_flip", "thermal_factory"];
var items = {
	...
	"Image_Enhancement"	: [ "thermal_inverse", "thermal_flip", "thermal_color" ],
	"Video_Enhancement" : [ "a_flicker" ],
	"Thermal_Settings"  : [ "thermal_agc", "thermal_mgcc", "thermal_mgcb", "thermal_edge_mode", "thermal_edge_level", "thermal_limit", "thermal_gamma", "thermal_denoise", "thermal_image" ],
	"Shutter_Settings"  : [ "thermal_shutter_mode", "thermal_shutter_time", "thermal_shutter_temp" ],
	"Factory_Reset"  : [ "thermal_factory" ]
};
var initialValue={};
var menu;
var source;
...
if( capInfo.image_sensor == 'imx178' || capInfo.image_sensor =="imx322" || capInfo.image_sensor =="imx226" || capInfo.image_sensor =="bt1120_720p") {
	hdr_not_supported=1;
}
...
//hdseo add thermal control start
	setSlider("thermal_mgcc"  , 0   , 1024);
	
	setSlider("thermal_mgcb"  , 0    , 511);
	setSlider("thermal_edge_level" , 0 , 1000);

	setSlider("thermal_limit"   , 0    , 255);
	setSlider("thermal_gamma"        , 1  , 5);
	setSlider("thermal_shutter_time" , 1    , 120);
	
	setSlider("thermal_shutter_temp"     , 1 , 10);
//hdseo add thermal control end
...
//			else $("#wdr_caution").css("color" , "#b900000");
	...
	if( menu == "Backlight_Switch" && capInfo.board_chipset == "amba_s2l66") {
        ...
        if(capInfo.image_sensor == 'imx307'||capInfo.image_sensor == 'ov4689'||capInfo.image_sensor == 'imx385_mipi'||capInfo.image_sensor == 'imx335_mipi'||capInfo.image_sensor == 'imx334_mipi'){
...
function dependence()    
{
	...
	if( CameraFunc[source].thermal_agc ) {
		$("[name=thermal_mgcc]").prop("disabled", true);	
		$("[name=thermal_mgcb]").prop("disabled", true);	
		disabledSlider("thermal_mgcc", 1) ;
		disabledSlider("thermal_mgcb", 1) ;
	} else {
		$("[name=thermal_mgcb]").prop("disabled", false);
		$("[name=thermal_mgcc]").prop("disabled", false);
		disabledSlider("thermal_mgcc", 0) ;
		disabledSlider("thermal_mgcb", 0) ;
	}

	/*if( CameraFunc[source].thermal_shutter_mode == 2 ) {
		$("[name=thermal_shutter_time]").prop("disabled", true);	
		$("[name=thermal_shutter_temp]").prop("disabled", true);	
		disabledSlider("thermal_shutter_time", 1) ;
		disabledSlider("thermal_shutter_temp", 1) ;
	} else */
	if( CameraFunc[source].thermal_shutter_mode == 0 ) {
		$("[name=thermal_shutter_time]").prop("disabled", true);	
		$("[name=thermal_shutter_temp]").prop("disabled", false);	
		disabledSlider("thermal_shutter_time", 1) ;
		disabledSlider("thermal_shutter_temp", 0) ;
	} else	{
		$("[name=thermal_shutter_time]").prop("disabled", false);	
		$("[name=thermal_shutter_temp]").prop("disabled", true);	
		disabledSlider("thermal_shutter_time", 0) ;
		disabledSlider("thermal_shutter_temp", 1) ;
	}
...
function initEvent(){
	...
	else if( menu =="Image_Enhancement" )
	{
		...
		*/
		getElement("thermal_inverse").on("change",function(){
			var value = $("#thermal_inverse").val();
			_ajax('thermal_inverse', value);
		});
	...
	else if( menu =="Thermal_Settings")
	{
		$("#vin_source option:eq("+MJ.id+")").prop("selected", "selected");
		getElement("thermal_agc").on("change",function(){
			var value = $("#thermal_agc").val();
			_ajax('thermal_agc', value);
		});
		getElement("thermal_mgcc").on("slidechange", function(){
			var value = getElement("thermal_mgcc").slider('value'); 
			_ajax('thermal_mgcc', value);
		});
		getElement("thermal_mgcb").on("slidechange", function(){
			var value = getElement("thermal_mgcb").slider('value'); 
			_ajax('thermal_mgcb', value);
		});

		getElement("thermal_edge_mode").on("change",function(){
			var value = $("#thermal_edge_mode").val();
			_ajax('thermal_edge_mode', value);
		});
		getElement("thermal_edge_level").on("slidechange", function(){
			var value = getElement("thermal_edge_level").slider('value'); 
			_ajax('thermal_edge_level', value);
		});

		getElement("thermal_limit").on("slidechange", function(){
			var value = getElement("thermal_limit").slider('value'); 
			_ajax('thermal_limit', value);
		});

		getElement("thermal_gamma").on("slidechange", function(){
			var value = getElement("thermal_gamma").slider('value'); 
			_ajax('thermal_gamma', value);
		});
	...
	else if( menu =="Shutter_Settings")
	{
		$("#vin_source option:eq("+MJ.id+")").prop("selected", "selected");
		getElement("thermal_shutter_mode").on("change",function(){
			var value = $("#thermal_shutter_mode").val();
			_ajax('thermal_shutter_mode', value);
		});
		getElement("thermal_shutter_time").on("slidechange", function(){
			var value = getElement("thermal_shutter_time").slider('value'); 
			_ajax('thermal_shutter_time', value);
		});
		getElement("thermal_shutter_temp").on("slidechange", function(){
			var value = getElement("thermal_shutter_temp").slider('value'); 
			_ajax('thermal_shutter_temp', value);
		});

	}
	else if( menu =="Factory_Reset")
	{
		$("#vin_source option:eq("+MJ.id+")").prop("selected", "selected");
		getElement("thermal_factory").on("change",function(){
			var value = $("#thermal_factory").val();
			_ajax('thermal_factory', value);
		});
	}
...
function onClickCameraDefault()
{
	...
	case "Video_Enhancement":
			menunum = 6 ;
			break;
	case "Thermal_Settings":
			menunum = 7 ;
			break;
	case "Shutter_Settings":
			menunum = 8 ;
			break;
	case "Factory_Reset":
			menunum = 9 ;	
