...
settingList = [ "ae_en", "exp_level", "a_flicker", "ae_meter", "awb_en", "awb_mode", "awb_method", "r_gain", "g_gain", "b_gain", "hue", "saturation", "brightness", "contrast", "sharpness", "mctf3dnr", "local_exp", "blc", "hdr", "hdr_level", "dnmode", "shutter", "sshutter", "mirror", "flip", "cam_gain","tdn_bw_level","tdn_color_level","tdn_resp"];
var items = {
...
function initUI() {
	...
	setSlider("sharpness"  , 1    , 100);
	setSlider("brightness" , 1 , 100);
	setSlider("contrast"   , 1    , 100);
	setSlider("hue"        , 1  , 100);
	setSlider("saturation" , 1    , 100);
...
function initValue()
{
	...
	for( var i=0; i<items[menu].length;i++) {
		...
		if( tag == "DIV" ){ //slider
			...
			type = obj.prop("type");
			if( type == "radio"){
				var obj_msg="[name=" + ID + "][value="+ initialValue[ID] + "]";
				$(obj_msg).prop("checked", true);
			}	
		}
		else if( tag == "SELECT"){
			obj.val(initialValue[ID]);
		}
	}
}
...
function dependence()    
{
	...
	if( typeof(CameraFunc.hdr) != "undefined" ){
		var hdr_dependency = CameraFunc.hdr > 0 ? true : false;
		...
		if( $(".select_minor").attr("id") == "Exposure_Settings"){
			...
			disabledSlider("exp_level", hdr_dependency || !CameraFunc.ae_en);
		}
	}
}
