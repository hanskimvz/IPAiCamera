...
var items = {
	...
	"Day_Night_Settings": [ "dnmode", "tdn_bw_level", "tdn_color_level",  "color_hour", "color_min", "bw_hour", "bw_min" , "wlmode" , "irmode" ],
...
function initUI() {
	...
	setSlider("tdn_color_level"   , 0    , 10);  // 이상민 과장 요청 60~120 > 0~120 , 40~100, 0~100
	setSlider("tdn_bw_level"   , 0    , 10);
...
function checkWLMode()
{
	if( capInfo.camera_module != "ov_isp" || capInfo["oem"] != 24 || capInfo.have_cds == 0)
		$("[class=whiteled_mode]").hide(); // overview mode is show
}
function checkIRMode()
{
	if( capInfo.camera_module != "ov_isp" || capInfo["oem"] != 24 || capInfo.have_cds == 0)
		$("[class=ir_mode]").hide(); // overview mode is show
}
...
function initEvent(){
	...
	else if( menu =="Day_Night_Settings")
	{
		...
		getElement("wlmode").on("click", function(){
			var value = $("[name=wlmode]:checked").val();
			_ajax("wlmode", value);
		});
		getElement("irmode").on("click", function(){
			var value = $("[name=irmode]:checked").val();
			_ajax("irmode", value);
		});
