...
var items = {
	...
	"Image_Adjustment"	: [ "sharpness","saturation"],
	"Exposure_Settings"	: [ "ae_en", "ae_shutter", "ae_gain", "ae_bright", "sshutter"],
...
//	setSlider("dn_threshold"   , 0    , 20);
	setSlider("tdn_color_level"   , 0    , 20);  // 이상민 과장 요청 60~120 > 0~120 , 40~100, 0~100 > 0~20
	setSlider("tdn_bw_level"   , 0    , 20); // 0~20
...
/*	if( capInfo['have_cds'] == 0 ) {
	...
	{
		if( CameraFunc[source]['hdr'] == 3 && $("#wdr_caution").length == 0 ) {
...
function checkWhiteBalance(){
	...
	if( $("[name=awb_mode]").val()  == 2 ) {  //one push
...
//	$("#agc_tr").show();
	...
	if( $("[name=ae_en]").val() == 1 ){  // manual (not used)
/*		$("#shutter_tr").show();
		$("#ae_iris_tr").show();
		$("#ae_gain_tr").show();
...
//		$("#sshutter_tr").hide();
	...
	else if( $("[name=ae_en]").val() == 3){  // iris priority (not used)
//		$("#ae_iris_tr").show();
	}
	else if( $("[name=ae_en]").val() == 4 ){  // bright (not used)
/*		$("#ae_bright_tr").show();
...
function checkHdr(){
		$("#support_hdr_level").hide();
	if( $("[name=hdr]").val() == 3 ){
		$("#support_hdr_level").show();
	}
/*	if( $("[name=hdr]").val() == 0 ){	
		$("#support_blc").show();
		//$("#hdr_level").hide();
...
//		setEvent( "agc" , "change" );	
	...
	else if( menu =="Day_Night_Settings")
	{
		...
		//setEvent( "dn_threshold" , "slidechange" );
