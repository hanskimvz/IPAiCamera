...
settingList = [ "ae_en", "exp_level", "a_flicker", "ae_meter", "awb_en", "awb_mode", "awb_method", "r_gain", "g_gain", "b_gain", "hue", "saturation", "brightness", "contrast", "sharpness", "mctf3dnr", "local_exp", "blc", "hdr", "hdr_level", "dnmode", "shutter","shutter_limit", "sshutter", "mirror", "flip", "cam_gain","tdn_bw_level","tdn_color_level","tdn_resp","ir_enabled","ir_strength"];
var items = {
	//menuTitle 		: [ items in menutitle ]
	"Image_Adjustment"	: [ "sharpness", "brightness", "contrast", "saturation", "hue"], 
	"Exposure_Settings"	: [ "ae_en", "exp_level", "local_exp", "ae_meter", "shutter","shutter_limit", "sshutter" , "cam_gain", "fixed_shutter" ],
	"Day_Night_Settings": [ "dnmode", "tdn_bw_level", "tdn_color_level", "tdn_resp", "color_hour", "color_min", "bw_hour", "bw_min","ir_enabled","ir_strength"],
...
function initUI() {
	if(capInfo['oem'] == 5 || capInfo['oem'] == 12 || capInfo['oem'] == 11 || capInfo['oem'] == 13 || capInfo['oem'] == 1 || capInfo['oem'] == 10 || capInfo['oem'] == 25 ){
    ...
    if (capInfo.pwm == 0){
        $("[name=iir_strength]").css("display","none");
    }
	debug("initUI");
   
    // if(capInfo['oem'] != 9 )
    //     $("#shutter_limit_tr").css("display" , "none");
    if(capInfo['oem'] == 3 && devInfo['model_num'] == "NDLP-SLAH8") 
        $("#slow_shutter_limit_tr").remove();
	else 
    {
        $("#max_shutter_limit_tr").remove();
    }
    
    if(capInfo['oem'] != 10 && capInfo['oem'] != 11 && capInfo['oem'] != 13 && capInfo['oem'] != 25) //UDP 7종
    {
        $("#m_shutter").css("display","none");
    }


    //if(capInfo["oem"] != "IV") 	$("#m_shutter").css("display" , "none");

  $("[name=shutter_limit_min]").empty();
  $("[name=shutter_limit_max]").empty();
  $("[name=shutter_limit_max]").append('<option value=15> 1/15 </option>');
  $("[name=shutter_limit_max]").append('<option value=25> 1/25 </option>');
  $("[name=shutter_limit_max]").append('<option value=30> 1/30 </option>');
  $("[name=shutter_limit_max]").append('<option value=50> 1/50 </option>');
  $("[name=shutter_limit_max]").append('<option value=60> 1/60 </option>');
  $("[name=shutter_limit_max]").append('<option value=100> 1/100 </option>');
  $("[name=shutter_limit_max]").append('<option value=120> 1/120 </option>');
  $("[name=shutter_limit_max]").append('<option value=240> 1/240</option>');
  $("[name=shutter_limit_max]").append('<option value=480> 1/480 </option>');
  $("[name=shutter_limit_max]").append('<option value=600> 1/600 </option>');
  $("[name=shutter_limit_min]").append('<option value=1000> 1/1000 </option>');
  $("[name=shutter_limit_min]").append('<option value=2000> 1/2000 </option>');
  $("[name=shutter_limit_min]").append('<option value=4000> 1/4000 </option>');
  $("[name=shutter_limit_min]").append('<option value=8000> 1/8000 </option>');
  $("[name=shutter_limit_min]").append('<option value=16000> 1/16000 </option>');
  $("[name=shutter_limit_min]").append('<option value=32000> 1/32000 </option>');
	...
	if(capInfo.image_sensor =="imx226" || capInfo.image_sensor =="imx412_mipi" ||corridor_mode != 0){
   ...
   setSlider("exp_level"  , 25   , 400);
	...
	if(capInfo['oem'] == 9 && capInfo.image_sensor == "imx464_mipi"){
	setSlider("sharpness"  , 0    , 255);
	}
	else{
	...
	if( $(".select_minor").attr("id") == "Day_Night_Settings"){
        ...
        if(capInfo['oem'] != 5 && capInfo['oem'] != 12 && capInfo['oem'] != 11 && capInfo['oem'] != 13 && capInfo['oem'] != 1 && capInfo['oem'] != 10 && capInfo['oem'] != 25 && capInfo['oem'] != 3){
		    $("[name=ir_mode]").css("display","none");        
        }
	}
...
//			else $("#wdr_caution").css("color" , "#b900000");
	...
	if( menu == "Backlight_Switch" && capInfo.board_chipset == "amba_s2l66") {
        ...
        if( $("#hdr option[value=1]").length == 0){
 		...
 		if(capInfo.image_sensor == 'ov4689' 
		|| capInfo.image_sensor == 'imx334_mipi'
		|| capInfo.board_chipset == "amba_s6lm33"
		|| capInfo.board_chipset == "amba_s6lm55"
		|| capInfo.board_chipset == "amba_cv22s88"){
		    $("[name=support_hdr_level]").css("display","none");        
        }
		else{
//		if(capInfo.image_sensor == 'imx335_mipi'){
			$('#hdr_level').children('option').remove();
			$('#hdr_level').append("<option value='0'>LOW</option>");
			$('#hdr_level').append("<option value='1'>MIDDLE</option>");
			$('#hdr_level').append("<option value='2'>HIGH</option>");
		}
    }
	...
	if( capInfo.image_sensor == 'ar0230' ) {
		...
		getElement("cam_gain").empty().append(content);
			...
			|| capInfo.image_sensor =="imx335_mipi" 
			|| capInfo.image_sensor =="imx334_mipi"
			|| capInfo.image_sensor =="imx415_mipi"
			|| capInfo.image_sensor =="imx274_mipi"
			|| capInfo.image_sensor =="imx464_mipi" ) {
		content += "<option value=0> 0 dB </option>";
		content += "<option value=1> 3 dB </option>";
		content += "<option value=2> 6 dB </option>";
		content += "<option value=3> 9 dB </option>";
		...
		getElement("cam_gain").empty().append(content);
	} else if( capInfo.image_sensor =="imx412_mipi" ) {
		content += "<option value=0> 0 dB </option>";
		content += "<option value=1> 3 dB </option>";
		content += "<option value=2> 6 dB </option>";
		content += "<option value=3> 9 dB </option>";
		...
		content += "<option value=12> 48 dB </option>";
		content += "<option value=13> 51 dB </option>";
		getElement("cam_gain").empty().append(content);
    ...
    { 
        $("#exp_level").attr("style","width : 120px !important");
    }
	...
	for( var i=0; i<items[menu].length;i++) {
		...
		if( tag == "DIV" ){ //slider
			...
			if( type == "radio"){
				...
				$("#"+ ID).prop("checked", initialValue[source][ID] == 1 ? true:false);
			}
		}
		else if( tag == "SELECT"){      
      obj.val(initialValue[source][ID]);
		}
    else if(ID == 'shutter_limit') {
      var isDefaultShutter_limit = false;
      var shutter_limit_max = parseInt(Number(initialValue[source][ID]) / 100000);
      if(shutter_limit_max == 0) {
        isDefaultShutter_limit = true;
        shutter_limit_max = 30;
      }
      $("[name=shutter_limit_max]").val(shutter_limit_max);
      var shutter_limit_min = isDefaultShutter_limit ? Number(initialValue[source][ID]) : Number(initialValue[source][ID]) - (shutter_limit_max * 100000);
      $("[name=shutter_limit_min]").val(shutter_limit_min);
    }
	}
	dependence();
}
...
function dependence()    
{
	if( CameraFunc[source].ae_en ) {
		$("#m_shutter").prop("disabled", false);
		$("[name=shutter_limit_min]").prop("disabled",false);
    $("[name=shutter_limit_max]").prop("disabled",false);

		if($('input:checkbox[id="fixed_shutter"]').is(":checked") == true)
		{
			$("[name=shutter]").prop("disabled", false);
      $("[name=shutter_limit_min]").prop("disabled",true);
      $("[name=shutter_limit_max]").prop("disabled",true);
		}
		else
        ...
        $("[name=shutter_limit_min]").prop("disabled",true);
        $("[name=shutter_limit_max]").prop("disabled",true);
        $("#fixed_shutter").prop("checked", false);
        $("#m_shutter").prop("disabled", true);
        if($('input:checkbox[id="fixed_shutter"]').is(":checked") == true)
            value = 1;
        else
            value = 0;
        _ajax('fixed_shutter', value);
	}
	...
	if((capInfo.camera_type == "PREDATOR_CLIENT" || capInfo.camera_module == "s2l_internal_isp") && menu == "Day_Night_Settings") {
...
{
	$("[name=iSchedule]").hide();
	$("[id=iColor_level]").hide();
	$("[id=iBw_level]").hide();
	$("[id=iResp]").hide();
	if( $("[name=dnmode]:checked").val() == 5 ) {   //Schedule
		$("[name=iSchedule]").show();
		$("[id=iColor_level]").hide();
		$("[id=iBw_level]").hide();
		$("[id=iResp]").hide();
	...
	{
		$("[name=iSchedule]").hide();
		$("[id=iColor_level]").show();
		$("[id=iBw_level]").show();
		$("[id=iResp]").show();

		if( $("[name=dnmode]:checked").val() == 0 || $("[name=dnmode]:checked").val() == 1 ) {
...
function initEvent(){
	...
	else if( menu == "Exposure_Settings")
	{
		...
		getElement("shutter_limit_min").on("change",function(){
      var value = Number(getElement("shutter_limit_max").val() * 100000) + Number(getElement("shutter_limit_min").val());
			//var value = getElement("shutter_limit_min").val();
			_ajax('shutter_limit', value);
		});
    getElement("shutter_limit_max").on("change",function(){
      var value = Number(getElement("shutter_limit_max").val() * 100000) + Number(getElement("shutter_limit_min").val());
			//var value = getElement("shutter_limit_max").val();
			_ajax('shutter_limit', value);
		});
		...
		getElement("fixed_shutter").on("change",function(){
			//var value = $('[name=fixed_shutter]:checked').val();
			if($('input:checkbox[id="fixed_shutter"]').is(":checked") == true)
				value = 1;
			else
				value = 0;
	...
	{
		if(capInfo.camera_type == "PREDATOR_CLIENT" || capInfo.camera_module == "s2l_internal_isp"){
			src = 0;
			MJ.id = src;
		...
		getElement("ir_strength").on("change", function(){
			var value = getElement("ir_strength").val();
			_ajax("ir_strength", value);
		});
	}
	else if( menu =="Backlight_Switch")
	{
