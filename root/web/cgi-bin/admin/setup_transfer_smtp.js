var menu = getLanguage("setup_smtp_config");
var settingList = ["enabled", "smtp_addr", "ssl_enable", "smtp_port","ssl_port", "id", "pass", "sender", "receiver", "title", "message", "detailedinfo", "cameraname", "eventrulename" ]; // radio 버튼 제외

function disabled(val ,cmd) {
	$("[name="+val + "]").find("select, input, button, textarea").each(function(i, e){
		var type = $(this).prop("id") ;
		$("#"+ type).prop("disabled", cmd);		
	});
}
function chkNumber(min, max, dft) {
	var objEv = event.srcElement;
	var objVal= objEv.value;
	var numPattern = /([^0-9])/;
	var numPattern = objEv.value.match(numPattern);
	if (numPattern != null) {
		settingFail(menu, "only input the number.");		
		objEv.focus();
		objEv.value = dft;
		return false;
	}
	if(objEv.value > max || objEv.value < min){
		objEv.setfocus=true;
		settingFail(menu, "your input valeu is out of range.");
		objEv.value = dft;
		return false;
	}
}

function initUI()
{
	if(SmtpInfo['enabled']== 0){
		disabled("SmtpContent",true);
        $("#btTest").prop("disabled",true);
	}
	else{
		disabled("SmtpContent", false);
        $("#btTest").prop("disabled",false);
	} 
	if(capInfo["oem"] != 2){
		$("#showcb").css("margin-left","145px");
		$("#showcb2").css("vertical-align","-webkit-baseline-middle");
	}
}
function initvalue()
{	
	var i, obj, tag, type, str;
	/*
	if(SmtpInfo["ssl_enable"]== 0){
		$("[name=ssl_enable][value=0]").trigger("click");
	} else {
		$("[name=ssl_enable][value=1]").trigger("click");
	}
	*/
	var value = new Value();
	value.setValue(settingList, SmtpInfo);
	console.log(value);

	settingList.forEach(function(id){
		obj = $("#" + id);
		if( obj.length == 0 ) {
			obj = $("[name=" + id +"]");
			if( obj.length == 0 ) {
				consoel.log("initValue() error!!");
				return;
			}
		}
		tag = obj.prop("tagName");
		type = obj.prop("type");
		if( type == "radio"){
			$("[name=" + id + "][value=" + SmtpInfo[id] + "]").attr("checked", true);
		} else if( type == "checkbox"){
			$("#"+ id).prop("checked", SmtpInfo[id] == 1 ? true:false);
		} else if( tag == "SELECT" || tag == "INPUT" ) {
			obj.val( SmtpInfo[id] );
		} else if( tag == "TEXTAREA") {
			str = SmtpInfo[id];
			obj.val(restoremsg(str));
		}
	});
	checkDependency("enable");	
}
function checkDependency(index) {
	if( index == "enable") {
		if(($("[name=ssl_enable]:checked").val()) == 1) {	
			$("#smtp_port_content").css( "display", "none");
			$("#ssl_port_content").css( "display", "block");
		} else if(($("[name=ssl_enable]:checked").val()) == 0) {		
			$("#smtp_port_content").css( "display", "block");
			$("#ssl_port_content").css( "display", "none");
		}
	}
	
}
function checkblank(val , msg){
	pop_msg = getLanguage(msg);
	if($("#" + val).val() == "" ){
		settingFail(menu, pop_msg);
		return false ; 
	}		
	return true;
}
function replacemsg(msg) {
	msg = msg.replace(/(?:\r\n|\r|\n)/g, '<br/>');
	return msg;
}
function restoremsg(msg) {
	msg = msg.split('<br/>').join("\r\n");
	return msg;
}
function initEvent() {
	$("[name=enabled]").click( function(e){
		if( e.delegateTarget.value == 0) {
            disabled("SmtpContent",true);
		    $("#btTest").prop("disabled",true);
		} else {
			disabled("SmtpContent", false); 
		    $("#btTest").prop("disabled",false);
		}
		$("#ssl_port").val(SmtpInfo["ssl_port"]);
		$("#smtp_port").val(SmtpInfo["smtp_port"]);
	});
	$("[name=ssl_enable]").click(function ( obj ) {				
		checkDependency("enable");	
		$("#ssl_port").val(SmtpInfo["ssl_port"]);
		$("#smtp_port").val(SmtpInfo["smtp_port"]);
	});
	$("#btOK").click(function(event) {
		if($("[name=enabled]:checked").val() == 1){
			if(!checkblank("smtp_addr", "msg_smtp_check_serveraddr")) return;			
			if(!checkblank("smtp_port", "msg_smtp_check_smtpport")) return;
			if(!checkblank("ssl_port", "msg_smtp_check_smtpport")) return;
			if(!checkblank("id", "msg_smtp_check_smtpid")) return;
			if(!checkblank("pass", "msg_smtp_check_smtppass")) return;
			if(!checkblank("sender", "msg_smtp_check_emailsend")) return;
			if(!checkblank("receiver", "msg_smtp_check_emailrev")) return;
		}
		var items = SmtpInfo;
		var cgiData = new Object();
		var data = null; 
		var newValue;
		var orgValue;
		var value = new Value();
		var change=0;

		var fail = settingList.some(function(e){
			newValue = value.getValue(e);
			if( $("#" + e).prop("type") == "checkbox") {
				if( $("#" + e).prop("checked") == true) {
					newValue = 1;
				} else {
					newValue  = 0;					
				}
			}
			oldValue = items[e];
			if( newValue != oldValue ) {
				cgiData[e] = newValue;
				if(e == "message") cgiData[e] = replacemsg(cgiData[e]);
				change ++;
			}
		});
		if(change == 0)
		{
			settingFail(menu, getLanguage("msg_nothing_changed"));
		}else
		{
			if( !fail){
				console.log(cgiData);
				_ajax(cgiData);
			}
		}
	});
	$("#btTest").click(function(event) {
		var value = new Value();
		var newValue;
		var ststr = {};
		var streamStr = '';
		settingList.some(function(e){
			streamStr = e;
			newValue = value.getValue(e);
			ststr[streamStr] = newValue;
		});
		$.ajax({
			type:"post",
			url: "/cgi-bin/admin/curltest.cgi?smtp",
			data: {test: ststr},
			timeout: 20000,
			beforeSend: function(){ progressUI(true); },
			success: function(msg){ progressUI(false); alert(msg); }, 
			error: function(err){ progressUI(false); alert("Connection timed out.\nPlease check if the port or other network settings are correct."); console.log(err); }
		});
	});
	
/*	$("#id").blur( function(obj){
		if( $("#id").val().length < 4 )  
		{ 
			pop_msg = getLanguage("msg_id_wrong");
			settingFail(menu, "ID is upper 4 character.");
			return false; 
		}
	});
	$("#pass").blur( function(obj){
		if( $("#pass").val().length < 4 )  		{ 
			pop_msg = getLanguage("msg_passwd_wrong");
			settingFail(menu, "ID is upper 4 character.");
			return false; 
		}
	});*/
	function check_length(val){
		if( val == "id" ){
			if( $("#id").val().length < 4 ){  				
				settingFail(menu, getLanguage("msg_id_wrong"));
				return false; 
			}
		}
		else if( val == "pass"){
			if( $("#pass").val().length < 4 ){  				
				settingFail(menu, getLanguage("msg_passwd_wrong"));
				return false; 
			}			
		}	
	}

/*	$("#smtp_addr, #smtp_port, #id, #pass, #sender, #receiver").blur( function(obj){
		if( checkblank(obj.target.id) == false) return ;
		check_length(obj.target.id);

		valAddress	= $("#ftp_addr").val();
		if( valAddress != ftp.address ){
			if( !validateURL(valAddress) && !validationAddress(valAddress) ) {
				alert(getLanguage("msg_ftp_wrong"));
				$("#ftp_addr").val(FtpInfo["ftp_addr"])
				return;
			}
		}
	});*/
}
function _ajax(data) {
	$.ajax({
		type:"get",
		url: "/cgi-bin/admin/transfer.cgi?msubmenu=smtp&action=apply",
		data: data,
		success: function(ret){
			var reg=/OK/g;
			if( reg.test(ret) ){
				settingSuccess(menu, getLanguage("msg_success"));
				refreshMenuContent();
			}else{
				settingFail(menu,getLanguage("msg_fail_retry"));
			}
		}, error: function(ret){
			settingSuccess(menu, getLanguage("msg_fail_retry"));
		}
	});
}

function onLoadPage() {	
	initEvent();
	initvalue();
	initUI();
}

$(document).ready( function() {
	onLoadPage();
});
